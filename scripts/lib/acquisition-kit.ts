...
export type AcquisitionRequest = {
  ...
  prompt: string;
  /** Ids of earlier requests whose generated bytes are fed back as identity/style references. */
  references?: string[];
...
export async function acquire(projectRoot: string, token: string, manifest: AcquisitionManifest): Promise<Record<string, AssetReceipt>> {
  ...
  async function reuse(request: AcquisitionRequest): Promise<boolean> {
    ...
    if (!generated || generated.input.prompt !== request.prompt || generated.input.aspect_ratio !== request.aspectRatio) return false;
  ...
  async function referenceData(id: string): Promise<string> {
    ...
    if (!receipt) throw new Error(`Reference ${id} must be acquired before the request that uses it`);
    const bytes = await readFile(join(projectRoot, receipt.sourcePath));
  ...
  for (const request of manifest.requests) {
    ...
    if (request.kind !== 'isolated') {
      ...
      assets[request.id] = { sourcePath: `${assetRoot}/${request.id}.jpg`, sha256: await fileDigest(target), provenance: 'replicate', predictionIds: [generated.id], generatedUrl };
      continue;
    }
