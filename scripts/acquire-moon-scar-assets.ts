...
async function reuseExisting(request: Request): Promise<boolean> {
  if (request.kind === 'isolated' && process.env.REBUILD_MATTE === '1') return false;
  ...
  try {
    ...
    const inspection = request.kind === 'isolated' ? await inspectAssetMatte(target, request.id === 'gu-yue-chun-neutral-v2' ? [255, 0, 255] : [0, 255, 102]) : undefined;
...
for (const request of requests) {
  ...
  const generationInput = {
    ...
    image_input: (request.references ?? []).map((reference) => reference === 'legacy-gu-yue-chun-reference-v1' ? chunReferenceData : reference),
    max_images: 1,
  ...
  const prepared = await prepareLocalMatte(generatedBytes, await download(transparentUrl), request.id === 'gu-yue-chun-neutral-v2' ? [255, 0, 255] : [0, 255, 102]);
