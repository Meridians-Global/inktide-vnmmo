...
const domainPath = join(inktideRoot, '.meridians-local/record/docs/domains/N-IMP-mtwby1q31jbg.json');
const evidenceDirectory = join(projectRoot, 'productions/spider-man-memory-between-us-v1/evidence');
const outputDirectory = join(inktideRoot, 'tmp/vn-lab-assets/spider-man-memory-between-us-v1');
...
if (!configuredToken) throw new Error('REPLICATE_API_TOKEN is missing from the inktide .env');
...
async function existingAsset(id: string, extension: 'jpg' | 'png'): Promise<{ generatedUrl: string; transparentUrl?: string; sourcePath: string; sha256: string; predictionIds: string[] } | undefined> {
  ...
  try {
    ...
    return {
      ...
      sourcePath: `vn-lab-assets/spider-man-memory-between-us-v1/${targetName}`,
...
async function prepareStageCrop(): Promise<{ sourcePath: string; sha256: string; preparation: Record<string, unknown> }> {
  ...
  return {
    ...
    preparation: {
      ...preparation,
      sourcePath: `vn-lab-assets/spider-man-memory-between-us-v1/${sourceName}`,
